package com.stas.parceldelivery.commons.constants;

public class Routes {
	public final static String OrderCreated = "OrderCreated";
	public final static String LocationChanged = "LocationChanged";
	public final static String OrderAssignment = "OrderAssignment";
	public final static String OrderCancelled = "OrderCancelled";
	public final static String OrderStatusChanged = "OrderStatusChanged";
	public final static String OrderUpdated = "OrderUpdated";
}
